function [points, pos, faceInds] = intersectMultiLineMesh3d(lines, vertices, faces, varargin)
...
lines = reshape(lines, 1, size(lines, 2), n_lines);
...
a = -dot(repmat(n, 1, 1, n_lines), w0, 2);
b = dot(repmat(n, 1, 1, n_lines), repmat(dir, size(n, 1), 1, 1), 2);

valid = abs(b) > tol & repmat(vectorNorm3d(n),1, 1, n_lines) > tol;
...
points = bsxfun(@plus, lines(:, 1:3,:), bsxfun(@times, pos, dir));
...
w   = points - t0;
wu  = dot(w, repmat(u,1,1,n_lines), 2);
wv  = dot(w, repmat(v,1,1,n_lines), 2);
...
pos(ind2) = NaN;
ind2 = repmat(ind2, 1,3,1);
points(ind2) = NaN;

% keep only interesting points
inds = ~ind1 & ~ind2 & repmat(valid,1,3,1);

points_cell = mat2cell(points, [size(points,1)], size(points,2), n_lines);
points(inds) = NaN;

pos = pos(inds(:,1,:));
faceInds = find(inds(:,1,:));
